...
for i = 1:length(filelist)
    ...
    [filepath,name,ext] = fileparts(img_fn)

    human_map = imread(fullfile(human_folder, [name '.png']));
    parsing_map = imread(fullfile(parsing_folder, [name '.png']));
    human_edge = imread(fullfile(edge_folder, [name '.png']));
    human_edge = double(human_edge) / 255;
    
    parsing_score_data = load(fullfile(parsing_folder, [name '.mat']));
    ...
    fid = fopen(fullfile(instance_folder, [name '.txt']), 'w');
    counter = 0;
    for k = 1:class_num
        scores = [];
        cur_counter = counter;
    ...
    imwrite(uint8(instance_map), colormap, fullfile(instance_folder, [name '.png']));
end    
